...
class GUI:
    ...
    def apicallworker(self):
        ...
        if len(self.username) == 0 or len(self.password) < 2:
            self.statusText1.set('Enter a username and password')
            return

        if len(self.query) > 1 and 'time' in self.query:
            myquery = 'select * where ' + self.query
        else:
            self.statusText1.set('Query Must be Set with Time Range')
            return
                    ...
                    for MetaData in event['results']['fields']:
                        if myCurrentGroup != MetaData['group']:
                            ...
                            if eventsCount == -1:
                                ...
                                self.myEvent = self.myEvent[:len(self.myEvent) - 2]
                                self.myEvent = self.myEvent + '}'
                                file.write(self.myEvent + '\n')
                        ...
                        if MetaData['type'] == "time":
                            ...
                            if CurrentTime != LastTime:
                                ...
                                LastTime = CurrentTime
                                self.myEvent = 't=t.plus(' + str(TimeDelta) + ' seconds) \n' + self.myEvent
                        if obfuscate == 1:
                        ...
                        self.myEvent = self.myEvent + MetaData['type'] + '=' + separator + str(MetaData['value']) + separator + ', '
    ...
    def readme(self, event):
        ...
        webbrowser.open('https://s01-git-01.mss.leidos.com/siem/rsa-sa/nw-esarules/wikis/Test-ESA-Rules', new=2)
                ...
                if len(self.outputfile) > 1:
                    ...
                    if result == 'no':
                        try:
                            ...
                            choice = tkMessageBox.askquestion("ERROR!", "There was an error attempting to remove the file. Do you still want to QUIT?\n\n" + error + "\n\n", icon='warning')
                            if choice == 'no':
                                return
                            else:
                                root.destroy()
